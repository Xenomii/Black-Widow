    ...
    if 'declare' in strhURL.lower() and '@' in strhURL and '=' in strhURL and 'set' in strhURL.lower() and strHexIdentifier in strhURL:
        ...
        for strTmpUlistItem in urllist:
            ...
            if strTmpHex != "":
                ...
                if len(strTmpHex) > 1:
                    strHexDecoded = strHexDecoded + binascii.unhexlify(strTmpHex).decode(
                        'unicode_escape') + strhURL.replace(strHexDecoded + strHexIdentifier + strTmpHex, "", 1)
                else:
...
def fileProcess(strInputFpath, strFileName, strOutPath):
    ...
    if boolSingleFile == True:
        ...
        strOutPath = strOutPath + strFileName
            ...
            for r_row in reader:  # loop through each row of input
                    ...
                    if len(r_row) / columnCount >= 2:
                        ...
                        for testColumns in r_row:
                            ...
                            if '\n' in testColumns:
                                ...
                                queuedRows[len(queuedRows) - 1][intListCount - 1] = testColumns[
                                                                                    0:testColumns.find("\n")]
                                break
                ...
                for row in queuedRows:
                    ...
                    for column in row:
                        ...
                        if boolIIS == True and intColumnCount == 1 and (
                                "#Software:" in saniColumn or "#Version:" in saniColumn or "#Date:" in saniColumn):
                            boolExcludeRow = True
                        ...
                        if boolExpectDefaultFormat == True and intColumnCount == 6 and row[6].isnumeric() == True and \
                                row[
                                    7].isnumeric() == True:  # if this is the request column and next two columns are numeric then
                            boolRequestEnding = True  # Things line up formatting wise that we don't need to check for escape characters
                        elif boolExpectDefaultFormat == True and intColumnCount == 3 and "[" == row[4][
                                                                                                :1]:  # if the column after next is the datetime field then we need to merge the next field with this one
                            boolQuoteRemoved = True
                            boolEscapeChar = True
                        elif '\\' in saniColumn:
                            if (
                                    boolDateCoverted == True):  # if we have made it past the user name and date field (only need escape character checks for fields with user provided input). Example: domainname\x5Cryan.boyle
                                if right(saniColumn, 1) == "\\" and boolDeobfuscate == False:
                                    boolQuoteRemoved = True
                                saniColumn = str.replace(saniColumn, "\\", "")  # remove escape character
                                boolEscapeChar = True
                        elif boolExpectDefaultFormat == True and intColumnCount == 6 and (
                                'GET' in saniColumn or 'POST' in saniColumn or 'PUT' in saniColumn or 'HEAD' in saniColumn or 'PUT' in saniColumn or 'DELETE' in saniColumn) and boolRequestEnding == False:
                            ...
                            boolEscapeChar = True  # specific way to identify the request column and combine
                        elif boolExpectDefaultFormat == True and intColumnCount == columnCount and len(
                                row) - intColumnCount - skippedColumns != columnCount - intColumnCount:
                        ...
                        if boolEscapeChar == True and len(
                                row) > columnCount and boolSkipColumn == False and boolQuoteRemoved == True:  # escaped character and column mismatch
                            ...
                            if len(row) - intColumnCount - skippedColumns != columnCount - intColumnCount:  # more columns than what is expected so combine next column
                                boolSkipColumn = True
                        elif boolSkipColumn == True and (
                                len(row) - intColumnCount - skippedColumns != columnCount - intColumnCount):  # still more columns than what is expected so combine next column
                            skippedColumns += 1
                            ...
                            if intCheckFirstUserInput == 0:
                                intCheckFirstUserInput = CheckRemainingColumns(row, intColumnCount,
                                                                               True)  # row, currentColumn, boolCheckNumeric
                            if intCheckFirstUserInput >= intColumnCount:  # check for special chars followed by number (In apache logs this is the first non system/user provided column that is followed by a status code)
                                outputRow = outputRow + " " + saniColumn  # continue column and add separator char back
                            elif boolQuoteRemoved == True and CheckRemainingColumns(row, intColumnCount,
                                                                                    False) <= intColumnCount and not (
                                    intColumnCount - skippedColumns == columnCount and len(
                                    row) - intColumnCount != 0):  # check for special chars ensuring we don't close the column if there is still items to add
                                ...
                                outputRow = outputRow + " " + saniColumn  # continue column and add separator char back
                        elif boolSkipColumn == True and len(
                                row) - intColumnCount - skippedColumns == columnCount - intColumnCount:  # New columns are just right
                            skippedColumns += 1
                            outputRow = appendQuote(
                                outputRow) + ',"' + saniColumn + '"'  # Close column and add new column
                            boolSkipColumn = False
                            intWriteCount += 1
                            ...
                            if boolEscapeChar == True:
                                ...
                                boolSkipColumn = True
                            elif intColumnCount - skippedColumns >= columnCount and len(
                                    row) == intColumnCount and columnCount != len(
                                    row):  # we've got too many columns. Mash last one together
                                outputRow = appendQuote(
                                    outputRow) + ',"' + saniColumn + '"'  # Close column and add final column
                    ...
                    if right(outputRow, 1) != '\"':
                        ...
                        if boolOutputUnformatted == True:
                            with io.open(strOutPath + ".Unformatted", "a",
                                         encoding=outputEncoding) as fU:  # Unformatted output that eluded a final quote
                                fU.write(outputRow + "\n")
